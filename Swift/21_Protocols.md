...
> NOTE
> final 클래스는 하위 클래스 될 수 없으므로 `final` 수식어로 표시된 클래스에 `required` 수식어를 프로토콜 초기화 구문 구현에 표시할 필요가 없습니다. `final` 수식어에 대한 자세한 내용은 [재정의 방지 (Preventing Overrides)](https://docs.swift.org/swift-book/LanguageGuide/Inheritance.html#ID202) 를 참고 바랍니다.
